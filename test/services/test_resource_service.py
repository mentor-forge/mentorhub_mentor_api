...
class TestResourceService(unittest.TestCase):
    ...
    @patch("src.services.resource_service.Config.get_instance")
    @patch("src.services.resource_service.MongoIO.get_instance")
    def test_get_resources_returns_all(self, mock_get_mongo, mock_get_config):
        """Test get_resources returns all documents via MongoIO.get_documents."""
        mock_config = MagicMock()
        mock_config.RESOURCE_COLLECTION_NAME = "Resource"
        mock_get_config.return_value = mock_config
        ...
        mock_mongo.get_documents.return_value = [
            {"_id": "507f1f77bcf86cd799439011", "name": "resource1"},
            {"_id": "507f1f77bcf86cd799439012", "name": "resource2"},
        ]
        mock_get_mongo.return_value = mock_mongo

        result = ResourceService.get_resources(self.mock_token, self.mock_breadcrumb)

        self.assertIsInstance(result, list)
        self.assertEqual(len(result), 2)
        mock_mongo.get_documents.assert_called_once_with(
            "Resource", match={}, sort_by=[("name", 1)]
        )

    @patch("src.services.resource_service.Config.get_instance")
    @patch("src.services.resource_service.MongoIO.get_instance")
    def test_get_resources_name_filter(self, mock_get_mongo, mock_get_config):
        """Test get_resources applies a case-insensitive partial name filter."""
        mock_config = MagicMock()
        mock_config.RESOURCE_COLLECTION_NAME = "Resource"
        mock_get_config.return_value = mock_config
        ...
        mock_mongo = MagicMock()
        mock_mongo.get_documents.return_value = [
            {"_id": "507f1f77bcf86cd799439011", "name": "test-resource"}
        ]
        mock_get_mongo.return_value = mock_mongo
