...
class ResourceService:
    ...
    @staticmethod
    def get_resources(token, breadcrumb, name=None):
        """
        Get all resource documents, sorted by name ascending.
        ...
        Returns:
            list: All matching Resource documents
        """
        try:
            ...
            logger.info(
                f"Retrieved {len(resources)} resources for user {token.get('user_id')}"
            )
            return resources
        except Exception as e:
